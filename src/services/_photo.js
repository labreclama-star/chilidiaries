...
// до 1600 px по длинной стороне и ~0.5 МБ, формат на выходе — JPEG. Так
// экономим место и исходящий трафик Supabase, а форма грузится быстрее.
//
...
// Параметры сжатия перед загрузкой. fileType принудительно 'image/jpeg':
// иначе PNG/WebP остались бы тяжёлыми PNG/WebP, а мы называем файл .jpg.
...
// основном потоке.
//
// Значения возвращены к исходным (после диагностики медленной публикации):
...
// после сжатия это всегда image/jpeg → .jpg. Исходное имя файла в путь больше
// не попадает (кириллица/пробелы в ключе дают "Invalid key"), путь состоит
// только из времени и случайной строки.
function extForMime(mime) {
  return EXT_BY_MIME[mime] || 'bin';
}

// Сжимает Blob в JPEG. Библиотека подключается динамическим import(), чтобы не
