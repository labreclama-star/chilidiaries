...
export default function AdminLights() {
  const { lights, adminAddLight, adminUpdateLight, adminDeleteLight } = useApp();
  ...
  function handleSubmit(e) {
    ...
    if (editing && editing.id) adminUpdateLight(editing.id, payload);
    else adminAddLight(payload);
    setEditing(null);
  }
  ...
  const confirming = confirmDeleteId ? lights.find((l) => l.id === confirmDeleteId) : null;

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 18 }}>
        <h1 className="admin-section-title" style={{ margin: 0 }}>Свет</h1>
        <button className="btn btn-primary" onClick={openCreate}>+ Добавить лампу</button>
      </div>
      ...
      <Modal isOpen={!!editing} onClose={() => setEditing(null)} wide>
        <h2>{editing && editing.id ? 'Редактировать лампу' : 'Добавить лампу'}</h2>
      ...
      <AdminConfirmDialog
        ...
        title="Удалить лампу?"
        message={`«${confirming?.name}» будет удалена из каталога.`}
        onConfirm={() => confirming && adminDeleteLight(confirming.id)}
      />
    </div>
  );
}
